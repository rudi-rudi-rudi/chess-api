...
import { createGame, getGame, toState } from './store.mjs';
...
app.get('/v2/games/:id/moves', async (req, reply) => {
  ...
  const moves = game.chess.moves(from ? { square: from, verbose: true } : { verbose: true });
...
app.post('/v2/games/:id/moves', async (req, reply) => {
  ...
  const { from, to, promotion = 'q', san } = req.body || {};
  if (!san && (!from || !to)) {
